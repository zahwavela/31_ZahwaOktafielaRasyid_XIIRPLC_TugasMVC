<div class="container mt-5">
    ...
    <div class="row">
        <div class="col-6">
            <button type="button" class="btn btn-primary btn-sm mb-3" onclick="add()">Tambah Data Guru</button>
            <h3>Data Guru</h3>
            <ul class="list-group">
                <?php foreach ($data['guru'] as $i) : ?>
                    <li class="list-group-item d-flex justify-content-between align-item-center">
                        <?= $i['nama']; ?>
                        <div class="float-end">
                            <a href="<?= BASE_URL ?>/guru/detail/<?= $i['id']; ?>" onclick="tes(this)" data-id="<?= $i['id']; ?>" class="btn btn-primary btn-sm">Detail</a>
                            <button type="button" class="btn btn-success btn-sm" onclick="ubah_data(this)" data-id="ubah_<?= $i['id']; ?>">Ubah</button>
                            <a href="<?= BASE_URL ?>/guru/hapus/<?= $i['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('yakin?');">Hapus</a>
                        </div>
                    </li>
                <?php endforeach; ?>
            </ul>
                ...
                <form action="<?= BASE_URL; ?>/guru/ubah" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jurusan" id="jurusan" class="form-control">
                            <?php
                                if($i['jurusan'] == 'TB'){
                                    echo '<option value="TB" selected>TB</option><option value="DPIB">DPIB</option>';
                                } else if($i['jurusan'] == 'DPIB'){
                                    echo '<option value="DPIB">DPIB</option><option value="RPL" selected>RPL</option>';
                                } else if($i['jurusan'] == 'RPL'){
                                    echo '<option value="RPL">RPL</option><option value="RPL">RPL</option>';
                                }
                            ?>
                        </select>
                    </div>
